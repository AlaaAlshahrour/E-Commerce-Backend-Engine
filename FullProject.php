...
namespace App\Http\Controllers\Api;class InventoryController extends Controller{protected InventoryService $inventoryService;public function __construct(InventoryService $inventoryService){$this->inventoryService = $inventoryService;}public function index(){$result = $this->inventoryService->getAll();if(isset($result['message'])){return ResponseHelper::jsonResponse($result['message']);}return response()->json(['data' => $result['data']]);}public function show(int $productId){$result = $this->inventoryService->getByProductId($productId);if(isset($result['message'])){return ResponseHelper::jsonResponse($result['message']);}return response()->json(['data' => $result['data']]);}public function update(int $productId,Request $request){$request->validate([
...
'user_id' => rand(1,1000),'status' => 'Completed','total_amount' => rand(50,500),'shipping_address' => 'Damascus','created_at' => now(),'updated_at' => now(),];if(count($rows)>= 5000){DB::table('orders')->insert($rows);$rows = [];}}DB::table('orders')->insert($rows);}}
