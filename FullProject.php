...
'balance' => $wallet->balance,'transactions' => $transactions,]);}public function topUp(TopUpRequest $request){$user = $request->user();$wallet = $user->wallet;if(! $wallet){$wallet = Wallet::create([
...
'data' => [
'product_id' => $inventory->product->id,'product_name' => $inventory->product->name,'price' => $inventory->product->price,'photo_url' => $inventory->product->photo_url,'quantity' => $inventory->quantity,],];}public function updateQuantity(int $productId,int $quantity): array{$inventory = $this->inventoryRepository->getByProductId($productId);if(! $inventory){return ['success' => false,'message' => 'Product not found in inventory'];}$this->inventoryRepository->updateQuantity($productId,$quantity);return ['success' => true,'message' => 'Inventory updated successfully'];}}
// ===== D:\Development\Laravel\E-Commerce-Backend-Engine\app\Services\OrderService.php =====
namespace App\Services;class OrderService{public function __construct(protected OrderRepository $orderRepository){}public function getUserOrders(): array{$orders = $this->orderRepository->getUserOrders(Auth::id());if($orders->isEmpty()){return ['success' => false,'message' => 'No orders found'];}return ['success' => true,'data' => $orders];}public function getOrderById(int $orderId): array{$order = $this->orderRepository->getOrderById($orderId,Auth::id());if(! $order){return ['success' => false,'message' => 'Order not found'];}return ['success' => true,'data' => $order];}public function createOrder(array $data): array{$user = Auth::user();$cart = $user->cart;if(! $cart){return ['success' => false,'message' => 'Cart is empty'];}$cartItems = $cart->cartItems()->with('product.inventory')->get();if($cartItems->isEmpty()){return ['success' => false,'message' => 'Cart is empty'];}$unavailable = $cartItems->filter(fn($item)=>($item->product->inventory?->quantity ?? 0)< $item->quantity);if($unavailable->isNotEmpty()){return [
'success' => false,'message' => 'Some products are out of stock','data' => $unavailable->map(fn($item)=> [
'product_id' => $item->product_id,'product_name' => $item->product->name,'requested' => $item->quantity,'available' => $item->product->inventory?->quantity ?? 0,]),];}$order = $this->orderRepository->createOrder($user->id,$data,$cartItems);return ['success' => true,'message' => 'Order created successfully','data' => $order];}public function updateStatus(int $orderId,string $status): array{$order = $this->orderRepository->getOrderById($orderId,Auth::id());if(! $order){return ['success' => false,'message' => 'Order not found'];}$allowedTransitions = [
...
'required' => $order->total_amount,'available' => $wallet->balance,'shortage' => $order->total_amount - $wallet->balance,],];}try{$transaction = DB::transaction(function()use($order,$wallet){$balanceBefore = $wallet->balance;$amount = $order->total_amount;$wallet->decrement('balance',$amount);$transaction = Transaction::create([
...
'date','total_orders','total_revenue','pdf_path','export_start_time','export_end_time',];protected $casts = [
...
namespace App\Models;class Product extends Model{use HasFactory;protected $guarded = [];public function category(){return $this->belongsTo(Category::class);}public function inventory(){return $this->hasOne(Inventory::class);}public function getPhotoUrlAttribute($value){return asset($value);}}
...
namespace App\Jobs;class ProcessDailySalesJob implements ShouldQueue{use Queueable;protected string $date;public function __construct(?string $date = null){$this->date = $date
? Carbon::parse($date)->toDateString(): Carbon::yesterday()->toDateString();}public function handle(): void{$targetDate = $this->date;$executionStart = microtime(true);$initialMemory = memory_get_usage(true);$exportStartTime = Carbon::now();Log::info('========== Daily Sales Job Started ==========');Log::info("Processing date:{$targetDate}");Log::info("Started at:{$exportStartTime}");$existingReport = DailySalesReport::where('date',$targetDate)->first();if($existingReport){Log::warning("Report already exists for{$targetDate}");return;}$totalOrders = 0;$totalRevenue = 0;$ordersData = [];$chunkCounter = 0;$dayStart = Carbon::parse($targetDate)->startOfDay();$dayEnd = Carbon::parse($targetDate)->endOfDay();Order::where('created_at','>=',$dayStart)->where('created_at','<',$dayEnd)->with('orderItems.product')->chunkById(1000,function($orders)use(&$totalOrders,&$totalRevenue,&$ordersData,&$chunkCounter){$chunkCounter++;$chunkMemoryBefore = memory_get_usage(true);Log::info("Processing Chunk 
Log::info("Chunk Size:{$orders->count()}orders");foreach($orders as $order){$totalOrders++;$totalRevenue += $order->total_amount;if(count($ordersData)< 200){$items = [];foreach($order->orderItems as $item){$items[] = [
'product_name' => $item->product->name,'quantity' => $item->quantity,'unit_price' => $item->unit_price,'subtotal' => $item->quantity * $item->unit_price,];}$ordersData[] = [
'id' => $order->id,'status' => $order->status,'payment_status' => $order->payment_status,'total_amount' => $order->total_amount,'created_at' => $order->created_at->format('Y-m-d H:i:s'),'items' => $items,];}}$chunkMemoryAfter = memory_get_usage(true);Log::info("Chunk 
round(($chunkMemoryAfter - $chunkMemoryBefore)/ 1024 / 1024,5).' MB');Log::info('Current Total Memory Usage: '.
round(memory_get_usage(true)/ 1024 / 1024,5).' MB');});Log::info('Daily sales processed successfully');Log::info("Total Orders:{$totalOrders}");Log::info("Total Revenue:{$totalRevenue}");$pdfData = [
'date' => $targetDate,'total_orders' => $totalOrders,'total_revenue' => $totalRevenue,'orders' => $ordersData,];$pdf = Pdf::loadView('pdf.daily-sales-report',$pdfData);$pdfPath = "public/daily-reports/daily-sales-{$targetDate}.pdf";Storage::put($pdfPath,$pdf->output());$exportEndTime = Carbon::now();$executionEnd = microtime(true);$executionTime = round($executionEnd - $executionStart,5);$finalMemory = memory_get_usage(true);$memoryUsed = round(($finalMemory - $initialMemory)/ 1024 / 1024,5);Log::info('========== Performance Metrics ==========');Log::info("Execution Time:{$executionTime}seconds");Log::info("Memory Used:{$memoryUsed}MB");Log::info("Peak Memory Usage: " . round(memory_get_peak_usage(true)/ 1024 / 1024,5). " MB");Log::info("Chunks Processed:{$chunkCounter}");Log::info("Export completed at:{$exportEndTime}");DailySalesReport::create([
'date' => $targetDate,'total_orders' => $totalOrders,'total_revenue' => $totalRevenue,'pdf_path' => $pdfPath,'export_start_time' => $exportStartTime,'export_end_time' => $exportEndTime,]);Log::info('Daily sales report created successfully');Log::info('========== Job Finished ==========');}}
...
return new class extends Migration{public function up(): void{Schema::create('cart_items',function(Blueprint $table){$table->id();$table->foreignIdFor(Cart::class)->constrained()->cascadeOnDelete();$table->foreignIdFor(Product::class)->constrained()->cascadeOnDelete();$table->integer('quantity');$table->timestamps();});}public function down(): void{Schema::dropIfExists('cart_items');}};
...
return new class extends Migration{public function up(): void{Schema::create('daily_sales_reports',function(Blueprint $table){$table->id();$table->date('date')->unique();$table->integer('total_orders')->default(0);$table->decimal('total_revenue',15,2)->default(0);$table->string('pdf_path')->nullable();$table->dateTime('export_start_time')->nullable();$table->dateTime('export_end_time')->nullable();$table->timestamps();});}public function down(): void{Schema::dropIfExists('daily_sales_reports');}};

// === [Seeders] ===
// ===== D:\Development\Laravel\E-Commerce-Backend-Engine\database\seeders\CartItemSeeder.php =====
namespace Database\Seeders;class CartItemSeeder extends Seeder{public function run(): void{CartItem::factory(100)->create();}}
...
namespace Database\Seeders;class DatabaseSeeder extends Seeder{use WithoutModelEvents;public function run(): void{DB::disableQueryLog();$this->call([
...
namespace Database\Seeders;class OrderSeeder extends Seeder{public function run(): void{$rows = [];for($i = 0;$i < 50000;$i++){$rows[] = [
'user_id' => rand(1,1000),'status' => 'completed','total_amount' => rand(50,500),'shipping_address' => 'Damascus','created_at' => now()->subDays(rand(0,3)),'updated_at' => now(),];if(count($rows)>= 5000){DB::table('orders')->insert($rows);$rows = [];}}DB::table('orders')->insert($rows);}}
...
'user_id' => $user->id,'status' => 'Processing','payment_status' => 'pending','total_amount' => 25.00,'created_at' => Carbon::yesterday(),]);$job = new ProcessDailySalesJob;$job->handle();$report = DailySalesReport::where('date',Carbon::yesterday()->toDateString())->first();$this->assertNotNull($report);$this->assertEquals(4,$report->total_orders);$this->assertEquals(250.00,$report->total_revenue);}}
