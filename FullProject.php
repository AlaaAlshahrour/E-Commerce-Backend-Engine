...
'time_seconds' => $executionTime,]);if($res['success']){GenerateInvoicePdfJob::dispatch($res['data']['order']->id);}return $res;}finally{$perUserLock->release();}}function makeTransaction(Wallet $wallet,$amount,Order $order): Model{$balanceBefore = $wallet->balance;$wallet->decrement('balance',$amount);$balanceAfter = $wallet->balance;$transaction = $wallet->transactions()->create([
...
'product_id' => $item->product_id,'quantity' => $item->quantity,'unit_price' => $item->product->price,])->toArray();$order->orderItems()->createMany($orderItems);foreach($cartItems as $item){$inventory = $inventories->get($item->product_id);$inventory->quantity -= $item->quantity;$inventory->save();}$cart->cartItems()->delete();return $order->load('orderItems.product:id,name,photo_url');}public function updateStatus(Order $order,string $status): Order{$order->status = $status;$order->save();return $order->fresh();}private function formatOrder(Order $order): array{return [
...
$sections = [
'Controllers' => app_path('Http/Controllers'),'Services' => app_path('Services'),'Models' => app_path('Models'),'Enums' => app_path('Enums'),'Providers' => app_path('Providers'),'Resources' => app_path('Http/Resources'),'Requests' => app_path('Http/Requests'),'Repositories' => app_path('Repositories'),'Helpers' => app_path('Helpers'),'Middleware' => app_path('Http/Middleware'),'Jobs' => app_path('Jobs'),'Processors' => app_path('Processors'),'Console' => app_path('Console'),'Migrations' => database_path('migrations'),'Seeders' => database_path('seeders'),'Factories' => database_path('factories'),'Bootstrap' => base_path('Bootstrap'),'Config' => base_path('config'),'Routes' => base_path('routes'),'Tests' => base_path('Tests'),'Resources_views_pdf' => base_path('resources/views/pdf'),'ApiCollections' => base_path('api-collections'),];foreach($sections as $sectionName => $path){if(! File::exists($path)){$this->warn("$sectionName directory not found,skipping...");continue;}$isYamlSection = $sectionName === 'ApiCollections';$files = File::allFiles($path);$files = array_filter($files,function($file)use($sectionName,$isYamlSection){if($isYamlSection){return in_array($file->getExtension(),['yaml','yml']);}if($sectionName === 'Migrations'){return $file->getExtension()=== 'php';}return $file->getExtension()== 'php';});usort($files,function($a,$b){return strcmp($a->getFilename(),$b->getFilename());});$content .= "\n
...
'role' => EnsureUserIsAdmin::class,]);})->withExceptions(function(Exceptions $exceptions): void{$requestContext = function(Request $request): array{return [
