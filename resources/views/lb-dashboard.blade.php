...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tabs { display: flex; gap: 0.4rem; margin-bottom: 1.5rem; border-bottom: 1px solid #334155; padding-bottom: 0; flex-wrap: wrap; }
        .tab { padding: 0.5rem 1rem; border-radius: 8px 8px 0 0; cursor: pointer; font-size: 0.82rem; border: 1px solid transparent; border-bottom: none; }
        .tab.active { background: #1e293b; border-color: #334155; color: #7dd3fc; }
        ...
        .auth-ok { color: #4ade80; } .auth-fail { color: #f87171; } .auth-pending { color: #fbbf24; }
        ...
        .btn-orange { background: #c2410c; } .btn-orange:hover:not(:disabled) { background: #ea580c; }
        .btn-gray { background: #374151; } .btn-gray:hover:not(:disabled) { background: #4b5563; }
        ...
        .node-card.down { border-color: #dc2626; opacity: 0.6; }
        ...
        .stat-label { color: #94a3b8; } .stat-value { color: #e2e8f0; font-weight: bold; }
        ...
        .log-time { color: #4ade80; } .log-host { color: #7dd3fc; } .log-ms { color: #fbbf24; }
        ...
        .box { background: #1e293b; border: 1px solid #334155; border-radius: 10px; padding: 1.2rem; margin-bottom: 1rem; }
        .box h3 { color: #7dd3fc; font-size: 0.9rem; margin-bottom: 1rem; }
        ...
        .progress-bar-bg { background: #0f1117; border-radius: 4px; height: 8px; margin: 0.8rem 0; }
        .progress-bar-fg { background: linear-gradient(90deg,#22d3ee,#7dd3fc); height: 8px; border-radius: 4px; transition: width 0.3s; }
        .progress-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)); gap: 0.6rem; }
        .progress-stat { background: #0f1117; border-radius: 8px; padding: 0.6rem; text-align: center; }
        .ps-value { font-size: 1.2rem; font-weight: bold; color: #7dd3fc; }
        ...
        .rl { color: #94a3b8; } .rv { color: #e2e8f0; font-weight: bold; }
        ...
        .summary-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 0.6rem; }
        .summary-item { text-align: center; background: #0f1117; border-radius: 8px; padding: 0.8rem; }
        .sv { font-size: 1.3rem; font-weight: bold; color: #7dd3fc; } .sl { font-size: 0.7rem; color: #64748b; }
        .stress-log { background: #0d1117; border: 1px solid #21262d; border-radius: 8px; padding: 0.8rem; height: 140px; overflow-y: auto; font-size: 0.75rem; }
        ...
        .slog-ok { color: #4ade80; } .slog-err { color: #f87171; }

        /* Comparison */
        .comparison-table { width: 100%; border-collapse: collapse; font-size: 0.82rem; margin-bottom: 1rem; }
        .comparison-table th { background: #1e293b; color: #7dd3fc; padding: 0.6rem 1rem; text-align: right; border: 1px solid #334155; }
        .comparison-table td { padding: 0.6rem 1rem; border: 1px solid #334155; }
        .comparison-table tr:nth-child(even) td { background: #1a2535; }
        .comparison-table tr:nth-child(odd) td { background: #151d2a; }
        .highlight { color: #4ade80; font-weight: bold; }
        .algo-card { background: #0f1117; border-radius: 8px; padding: 1rem; border: 1px solid #334155; }
        .algo-card h4 { font-size: 0.85rem; margin-bottom: 0.6rem; }
        .algo-card.winner { border-color: #4ade80; }
        .chart-bar-row { display: flex; align-items: center; gap: 0.8rem; margin: 0.4rem 0; font-size: 0.78rem; }
        .chart-label { width: 120px; color: #94a3b8; text-align: left; flex-shrink: 0; }
        .chart-bar-container { flex: 1; background: #1e293b; border-radius: 4px; height: 20px; }
        .chart-bar { height: 20px; border-radius: 4px; display: flex; align-items: center; padding: 0 6px; font-size: 0.7rem; font-weight: bold; transition: width 0.8s; }
        .chart-val { width: 70px; text-align: right; flex-shrink: 0; }

        /* Failure */
        .nodes-control { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1rem; margin-bottom: 1rem; }
        .node-control-card { background: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 1.2rem; }
        .node-control-card.running { border-color: #22d3ee; }
        .node-control-card.stopped { border-color: #dc2626; opacity: 0.7; }
        .node-ctrl-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
        .node-ctrl-name { font-weight: bold; color: #7dd3fc; }
        .status-dot { width: 10px; height: 10px; border-radius: 50%; }
        .dot-green { background: #4ade80; box-shadow: 0 0 6px #4ade80; }
        .dot-red { background: #f87171; box-shadow: 0 0 6px #f87171; }
        .failure-log { background: #0d1117; border: 1px solid #21262d; border-radius: 8px; padding: 0.8rem; height: 180px; overflow-y: auto; font-size: 0.78rem; margin-top: 1rem; }
        .flog-entry { padding: 3px 0; border-bottom: 1px solid #1e293b; font-size: 0.75rem; }
        .flog-time { color: #64748b; margin-right: 0.5rem; }
        .flog-info { color: #94a3b8; }
        .flog-warn { color: #fbbf24; }
        .flog-err { color: #f87171; }
        .flog-ok { color: #4ade80; }
    </style>
</head>
<body>
...
<div class="tab-content" id="tab-stress">
    <div class="box">
        ...
        <div class="config-grid">
            <div class="config-item"><label>عدد المستخدمين</label><input type="number" id="st-users" value="100" min="1" max="500"></div>
            <div class="config-item"><label>المدة (ثانية)</label><input type="number" id="st-duration" value="30" min="5" max="120"></div>
            ...
            <div class="config-item"><label>الفاصل الزمني بين الدُفعات (ms)</label><input type="number" id="st-interval" value="100" min="50" max="2000"></div>
        </div>
    ...
    <div class="box" id="stress-progress" style="display:none">
        ...
        <div class="progress-bar-bg"><div class="progress-bar-fg" id="st-progress-bar" style="width:0%"></div></div>
        <div class="progress-stats">
            <div class="progress-stat"><div class="ps-value" id="st-live-total">0</div><div class="ps-label">إجمالي</div></div>
            <div class="progress-stat"><div class="ps-value" id="st-live-rps" style="color:#4ade80">0</div><div class="ps-label">req/s</div></div>
            <div class="progress-stat"><div class="ps-value" id="st-live-ok" style="color:#4ade80">0</div><div class="ps-label">ناجح</div></div>
            <div class="progress-stat"><div class="ps-value" id="st-live-err" style="color:#f87171">0</div><div class="ps-label">فاشل</div></div>
            <div class="progress-stat"><div class="ps-value" id="st-live-avg">0ms</div><div class="ps-label">متوسط زمن</div></div>
        </div>
    ...
    <div class="box" id="stress-results" style="display:none">
        ...
        <div style="background:#0f1117;border-radius:8px;padding:1rem;margin-top:1rem">
            ...
            <div class="summary-grid">
                <div class="summary-item"><div class="sv" id="r-total">0</div><div class="sl">إجمالي</div></div>
                <div class="summary-item"><div class="sv" id="r-success" style="color:#4ade80">0</div><div class="sl">ناجح</div></div>
                <div class="summary-item"><div class="sv" id="r-errors" style="color:#f87171">0</div><div class="sl">فاشل</div></div>
                <div class="summary-item"><div class="sv" id="r-rps">0</div><div class="sl">req/s</div></div>
                <div class="summary-item"><div class="sv" id="r-avg">0ms</div><div class="sl">متوسط زمن</div></div>
                <div class="summary-item"><div class="sv" id="r-max">0ms</div><div class="sl">أقصى زمن</div></div>
                <div class="summary-item"><div class="sv" id="r-users">0</div><div class="sl">مستخدم متزامن</div></div>
                <div class="summary-item"><div class="sv" id="r-duration">0s</div><div class="sl">مدة الاختبار</div></div>
            </div>
        </div>
    </div>
...
<div class="tab-content" id="tab-compare">
    <div class="box">
        ...
        <div class="config-grid">
            <div class="config-item"><label>عدد الطلبات لكل خوارزمية</label><input type="number" id="cmp-requests" value="60" min="20" max="200"></div>
            <div class="config-item"><label>نوع الطلبات</label>
                <select id="cmp-mode">
        ...
        <div id="cmp-results" style="display:none">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.5rem">
                <div class="algo-card" id="rr-card">
                    ...
                    <div class="stat"><span class="stat-label">متوسط زمن الاستجابة</span><span class="stat-value" id="rr-avg">-</span></div>
                    <div class="stat"><span class="stat-label">أقصى زمن استجابة</span><span class="stat-value" id="rr-max">-</span></div>
                    <div class="stat"><span class="stat-label">Throughput (req/s)</span><span class="stat-value" id="rr-rps">-</span></div>
                    <div class="stat"><span class="stat-label">توزيع على العقد</span><span class="stat-value" id="rr-dist">-</span></div>
                </div>
                <div class="algo-card winner" id="lc-card">
                    <h4 style="color:#4ade80">✅ Least Connections (فعلي)</h4>
                    <div class="stat"><span class="stat-label">متوسط زمن الاستجابة</span><span class="stat-value" id="lc-avg">-</span></div>
                    <div class="stat"><span class="stat-label">أقصى زمن استجابة</span><span class="stat-value" id="lc-max">-</span></div>
                    <div class="stat"><span class="stat-label">Throughput (req/s)</span><span class="stat-value" id="lc-rps">-</span></div>
                    <div class="stat"><span class="stat-label">توزيع على العقد</span><span class="stat-value" id="lc-dist">-</span></div>
                </div>
            </div>
...
<script>
    ...
    const GROUPS = [
        {name:'🔐 Auth',color:'#1e3050',headerColor:'#3b82f6',endpoints:[
                {name:'Register',method:'POST',url:'/api/register',auth:false,body:{name:'Test',email:'test@test.com',password:'password',password_confirmation:'password'}},
        ...
        {name:'📦 Products',color:'#0f2d1e',headerColor:'#16a34a',endpoints:[
                {name:'All Products',  method:'GET', url:'/api/products',  auth:false},
                {name:'Show Product',  method:'GET', url:'/api/products/1',auth:false},
                {name:'Create Product',method:'POST',url:'/api/products',  auth:true,body:{name:'Test',price:100,category_id:1,description:'This is a test product'}},
            ]},
        ...
        {name:'📋 Orders',color:'#12102a',headerColor:'#6366f1',endpoints:[
                ...
                {name:'Checkout',  method:'POST',url:'/api/orders/checkout',auth:true,body:{shipping_address:'bagdad street'}},
            ]},
    ...
    let stTotal=0,stOk=0,stErr=0,stTotalMs=0,stMaxMs=0,stRpsCount=0,stStartTime=0,stRpsInterval=null;
    ...
    async function loginFirst(manual=false){
        ...
        if(!manual&&token){if(await verifyToken()){showAuthOk();return true;}token=null;localStorage.removeItem('lb_token');}
        try{
            const res=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({email:'test@test.com',password:'password'})});
            const data=await res.json();
            if(data.data?.token){token=data.data.token;localStorage.setItem('lb_token',token);showAuthOk();return true;}
            showAuthFail(data.message||'فشل');return false;
        }catch(e){showAuthFail(e.message);return false;}
    }
    async function verifyToken(){try{const r=await fetch('/api/me',{headers:{'Accept':'application/json','Authorization':'Bearer '+token}});return r.ok;}catch{return false;}}
    function showAuthOk(){document.getElementById('auth-status').innerHTML='<span class="auth-ok">✅ مسجّل دخول: test@test.com</span>';document.getElementById('token-preview').textContent='Token: '+token.substring(0,20)+'...';}
    function showAuthFail(msg){document.getElementById('auth-status').innerHTML=`<span class="auth-fail">❌ فشل: ${msg}</span>`;document.getElementById('token-preview').textContent='';}
    ...
    <div class="group">
        ...
        <div class="group-body" style="background:${g.color}">
            ${g.endpoints.map((ep,ei)=>{const sel=selectedEndpoint===ep;const mc=METHOD_COLORS[ep.method]||'#374151';return`<button class="action-btn" style="background:${mc}${sel?'ff':'55'};border:2px solid ${sel?'#fff':'transparent'}" onclick="selectEndpoint(${gi},${ei})"><span class="method-badge" style="background:${mc}">${ep.method}</span><span class="ep-name">${ep.name}</span><span class="ep-url">${ep.url}</span></button>`;}).join('')}
        </div>
    </div>`).join('');
    }
    ...
    function recordMonitor(host,ms,action,isError,method='GET'){
        ...
        if(clean){if(!monitorStats[clean])monitorStats[clean]={count:0,totalMs:0,lastMs:0,lastSeen:null,_lastTime:0};const s=monitorStats[clean];s.count++;s.totalMs+=ms;s.lastMs=ms;s.lastSeen=new Date().toLocaleTimeString('ar');s._lastTime=Date.now();}
        renderMonitorNodes();
    ...
    function renderMonitorNodes(){
        ...
        const hosts=Object.keys(monitorStats).sort();if(!hosts.length)return;
        grid.innerHTML=hosts.map(host=>{const s=monitorStats[host];const avg=s.count?Math.round(s.totalMs/s.count):0;const pct=successCount?Math.round((s.count/successCount)*100):0;const isActive=(Date.now()-s._lastTime)<800;return`<div class="node-card ${isActive?'active':''}"><div class="node-name">🖥 ${host}<span class="badge ${isActive?'badge-active':'badge-idle'}">${isActive?'نشط ◉':'خامل'}</span></div><div class="stat"><span class="stat-label">الطلبات</span><span class="stat-value">${s.count}</span></div><div class="stat"><span class="stat-label">التوزيع</span><span class="stat-value">${pct}%</span></div><div class="stat"><span class="stat-label">متوسط زمن</span><span class="stat-value">${avg}ms</span></div><div class="stat"><span class="stat-label">آخر استجابة</span><span class="stat-value">${s.lastMs}ms</span></div><div class="stat"><span class="stat-label">آخر طلب</span><span class="stat-value">${s.lastSeen||'-'}</span></div><div class="bar-bg"><div class="bar-fg" style="width:${pct}%"></div></div></div>`;}).join('');
    }

    function addMonitorLog(time,host,ms,method,action,isError){
        const log=document.getElementById('log');const e=document.createElement('div');const mc=METHOD_COLORS[method]||'#374151';
        ...
        log.prepend(e);while(log.children.length>150)log.removeChild(log.lastChild);
    }

    async function sendBurst(){await Promise.all(Array.from({length:20},()=>fetchRequest()));}
    function toggleAuto(){const btn=document.getElementById('auto-btn');if(autoTimer){clearInterval(autoTimer);autoTimer=null;btn.textContent='▶ تلقائي';btn.className='btn btn-green';}else{autoTimer=setInterval(fetchRequest,400);btn.textContent='⏹ إيقاف';btn.className='btn btn-red';}}
    function clearAll(){monitorStats={};total=0;successCount=0;errorCount=0;reqLast=0;['total','success','errors'].forEach(id=>document.getElementById(id).textContent='0');document.getElementById('nodes-grid').innerHTML='';document.getElementById('log').innerHTML='';}
    ...
    function randEp(pool){return pool[Math.floor(Math.random()*pool.length)];}

    async function stressFetchOne(pool){
        const ep=randEp(pool);if(ep.auth&&!token)return;
        const t0=Date.now();
        try{
            const h={'Accept':'application/json','Content-Type':'application/json'};if(ep.auth&&token)h['Authorization']='Bearer '+token;
            const opt={method:ep.method,headers:h};if(ep.body&&['POST','PUT','PATCH'].includes(ep.method))opt.body=JSON.stringify(ep.body);
            const res=await fetch(ep.url,opt);const ms=Date.now()-t0;const data=await res.json();
            const node=VALID_NODES.includes(data.node)?data.node:null;
            stTotal++;stTotalMs+=ms;if(ms>stMaxMs)stMaxMs=ms;stRpsCount++;
            if(res.ok&&node){stOk++;if(!stressNodes[node])stressNodes[node]={count:0,totalMs:0,lastMs:0};stressNodes[node].count++;stressNodes[node].totalMs+=ms;stressNodes[node].lastMs=ms;}else{stErr++;}
            addStressLog(new Date().toLocaleTimeString('ar'),node||'?',ms,ep.method,ep.name,!res.ok);
        }catch(e){stTotal++;stErr++;}
        updateStressLive();
    }

    async function startStressTest(){
        if(stressRunning)return;
        if(!token){const ok=await loginFirst();if(!ok){alert('يجب تسجيل الدخول');return;}}
        ...
        stressRunning=true;stressNodes={};stTotal=0;stOk=0;stErr=0;stTotalMs=0;stMaxMs=0;stRpsCount=0;stStartTime=Date.now();
        ...
        stRpsInterval=setInterval(()=>{document.getElementById('st-live-rps').textContent=stRpsCount;stRpsCount=0;},1000);
        const pi=setInterval(()=>{const el=(Date.now()-stStartTime)/1000;const pct=Math.min((el/duration)*100,100);document.getElementById('st-progress-bar').style.width=pct+'%';document.getElementById('st-time-left').textContent=`${Math.max(0,Math.round(duration-el))}s متبقي`;},500);
        stressTimer=setInterval(async()=>{if(!stressRunning)return;await Promise.all(Array.from({length:Math.min(users,20)},()=>stressFetchOne(pool)));},interval);
        setTimeout(()=>{stopStressTest();clearInterval(pi);},duration*1000);
    }

    function stopStressTest(){
        stressRunning=false;if(stressTimer){clearInterval(stressTimer);stressTimer=null;}if(stRpsInterval){clearInterval(stRpsInterval);stRpsInterval=null;}
        document.getElementById('st-start-btn').disabled=false;document.getElementById('st-stop-btn').disabled=true;
        document.getElementById('st-progress-bar').style.width='100%';document.getElementById('st-status-text').textContent='✅ اكتمل';
        showStressResults();
    }
    ...
    function updateStressLive(){
        ...
        const hosts=Object.keys(stressNodes).sort();if(!hosts.length)return;
        grid.innerHTML=hosts.map(node=>{const s=stressNodes[node];const avg=s.count?Math.round(s.totalMs/s.count):0;const pct=stOk?Math.round((s.count/stOk)*100):0;return`<div class="node-card active"><div class="node-name">🖥 ${node}<span class="badge badge-active">نشط ◉</span></div><div class="stat"><span class="stat-label">الطلبات</span><span class="stat-value">${s.count}</span></div><div class="stat"><span class="stat-label">التوزيع</span><span class="stat-value">${pct}%</span></div><div class="stat"><span class="stat-label">متوسط زمن</span><span class="stat-value">${avg}ms</span></div><div class="bar-bg"><div class="bar-fg" style="width:${pct}%"></div></div></div>`;}).join('');
    }
    ...
    function showStressResults(){
        ...
        document.getElementById('r-total').textContent=stTotal;document.getElementById('r-success').textContent=stOk;
        document.getElementById('r-errors').textContent=stErr;document.getElementById('r-rps').textContent=rps;
        document.getElementById('r-avg').textContent=avg+'ms';document.getElementById('r-max').textContent=stMaxMs+'ms';
        document.getElementById('r-users').textContent=document.getElementById('st-users').value;document.getElementById('r-duration').textContent=elapsed+'s';
        document.getElementById('results-nodes').innerHTML=hosts.map(node=>{const s=stressNodes[node];const avg=s.count?Math.round(s.totalMs/s.count):0;const pct=stOk?Math.round((s.count/stOk)*100):0;return`<div class="result-node"><h4>🖥 ${node}</h4><div class="result-stat"><span class="rl">الطلبات المعالجة</span><span class="rv">${s.count}</span></div><div class="result-stat"><span class="rl">نسبة التوزيع</span><span class="rv">${pct}%</span></div><div class="result-stat"><span class="rl">متوسط زمن</span><span class="rv">${avg}ms</span></div><div class="result-stat"><span class="rl">آخر استجابة</span><span class="rv">${s.lastMs}ms</span></div><div class="result-bar"><div class="result-bar-fill" style="width:${pct}%"></div></div></div>`;}).join('');
    }

    function addStressLog(time,node,ms,method,action,isError){
        const log=document.getElementById('stress-log');const e=document.createElement('div');const mc=METHOD_COLORS[method]||'#374151';
        e.className='slog-entry';e.innerHTML=`<span class="${isError?'slog-err':'slog-ok'}">${time}</span><span style="color:#7dd3fc">${node}</span><span style="color:#fbbf24">${ms}ms</span><span style="background:${mc};padding:1px 4px;border-radius:3px;font-size:0.65rem">${method}</span><span style="color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${action}</span>`;
        log.prepend(e);while(log.children.length>200)log.removeChild(log.lastChild);
    }
    ...
    async function runComparison(){
        ...
        if(!token){await loginFirst();if(!token){document.getElementById('cmp-btn').disabled=false;return;}}
    ...
    async function runBenchmark(pool, n){
        ...
        for(let b=0;b<batches;b++){
            const batch=Array.from({length:Math.min(10,n-b*10)},async()=>{
                const ep=randEp(pool);if(ep.auth&&!token)return null;
                const t0=Date.now();
                try{
                    ...
                    const res=await fetch(ep.url,opt);const ms=Date.now()-t0;
                    const data=await res.json();
                    return{ms,node:data.node||'unknown',ok:res.ok,ep:ep.name};
                }catch{return{ms:0,node:'error',ok:false,ep:''};}
            });
        ...
        const nodeCounts={};VALID_NODES.forEach(n=>nodeCounts[n]=0);
    ...
    function simulateRoundRobin(lcResults){
        ...
        const rrNodeCounts={};nodes.forEach(n=>rrNodeCounts[n]=0);
        const rrLatencies=[];
    ...
    async function restoreAllNodes() {
        ...
        // انتظر 3 ثوانٍ ثم حدّث الحالة
        setTimeout(async () => {
            await fetchNodeStatus();
            addFailureLog('🔄 تم تحديث حالة العقد', 'info');
        }, 3000);
    }
    ...
    async function startFailureMonitor() {
        ...
        // جلب الحالة الفعلية أولاً
        await fetchNodeStatus();

        // تحقق أن على الأقل عقدة واحدة تعمل
        ...
        failureNodes = {}; flTotal = 0; flOk = 0; flErr = 0; flRps = 0;
    ...
    function stopFailureMonitor() {
        ...
        if (flRpsInterval) { clearInterval(flRpsInterval); flRpsInterval = null; }
        document.getElementById('fail-stop-btn').disabled = true;
        addFailureLog('⏹ توقفت المراقبة', 'info');
    }
